...
#define SOBEL_SCALE 5.0f  // Adjust for edge strength (smaller = stronger edges)

// Globals set after loading
int width = 0;
int height = 0;

// ── Image loading / saving / memory ─────────────────────────────────
...
int main(int argc, char **argv) {
    ...
    omp_set_num_threads(8);  // Adjust to your CPU cores
    ...
    // Pipeline: grayscale -> invert -> blur (on inverted) -> edge detection (on blurred)
